...
def get_daily_historic_data_yahoo(ticker,
    ...
    try:
        ...
        str_yf_data = []
        for word in yf_data:   # Python 3, urlopen return object, and here we convert it to string
            str_yf_data.append(word.strip().decode('utf-8')) # utf-8 works in your case
        prices = []
        for y in str_yf_data:
            p = y.strip().split(',')
...
def insert_daily_data_into_db(data_vendor_id, symbol_id, daily_data):
    ...
    insert_str = ("%s, " * 11)[:-2]
    final_str = "INSERT IGNORE INTO daily_price (%s) VALUES (%s)" % (column_str, insert_str)
